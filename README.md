...
| 设置 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `已读进度显示为100%` | Checkbox | 关闭 | 已人工标记读完的书在 Notion 中显示为 100%，不修改微信读书真实进度 |
| `移出书架时删除` | Checkbox | 开启 | 不在微信读书当前书架中的书籍及其自动同步内容移入 Notion 回收站 |
| `同步划线和笔记` | Checkbox | 开启 | 将划线和个人想法按章节写入书籍正文 |
| `阅读统计起始年份` | Number | 2023 | 从该年份开始生成阅读统计 |

所有可编辑配置都只使用 Checkbox 或 Number，不使用文本或空字符串表示状态。`已应用配置码` 是同步器维护的数字，请勿手动修改。
