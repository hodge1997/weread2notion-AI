...
如果微信读书 Gateway 在读取某一本书的详情时临时返回 499 或其他可重试请求错误，普通同步会跳过这本书、保留它现有的 Notion 页面，并继续处理其他书籍；失败书籍会列在 Actions Summary 中，下一次同步会再次尝试。全量同步遇到这类失败会直接中止，不会先归档旧数据后生成不完整结果。
