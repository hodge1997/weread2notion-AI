...
- **适配模板关系**：支持书架、作者、分类、章节、日、周、月、年，以及可选的阅读记录数据库。
...
4. 重建年、月、周、日。
5. 重建作者、分类和书架。
6. 重建章节，并把划线与个人想法按章节写入书籍正文。
7. 写入可用的阅读记录。

普通增量同步也会比较 `/shelf/sync` 与 Notion 书架：Notion 中存在、但微信读书当前书架中不存在的条目，会连同关联章节、旧版独立笔记/划线行和当前页面正文一起移入回收站。清理动作只会在本次所需的微信读书请求全部成功后执行。
