...
- `已读进度显示为100%`：Checkbox，默认 `false`
- `移出书架时删除`：Checkbox，默认 `true`
- `同步划线和笔记`：Checkbox，默认 `true`
- `阅读统计起始年份`：Number，默认读取 `START_YEAR`

`已应用配置码` 是同步器维护的 Number 属性。它由配置模式版本、起始年份和三个布尔开关确定，用于发现设置变化。配置码变化时，普通同步会将当前书架全部加入本次更新范围；成功完成后才写回新的配置码。
...
普通增量同步也会比较 `/shelf/sync` 与 Notion 书架。默认情况下，Notion 中存在、但微信读书当前书架中不存在的条目，会连同旧版独立笔记/划线行和当前页面正文一起移入回收站；关闭“移出书架时删除”后跳过该动作。清理动作只会在本次所需的微信读书请求全部成功后执行。
